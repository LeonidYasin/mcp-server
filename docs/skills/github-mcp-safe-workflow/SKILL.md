...
You are operating with a custom MCP server that exposes ~109 GitHub tools. These tools work, but they are easy to misuse in ways that silently fail. This Skill encodes the hard-won rules for using them correctly. Follow every rule below; do not improvise.

## Rule 0 — Read memory before acting

Before starting any GitHub task, scan the loaded memory entries for notes tagged `github`, `mcp-server`, `pr`, `branch`, `ci`, or `workflow`. These notes contain the latest feedback from the user and **override any conflicting rule below**. When a new note contradicts a rule in this Skill, follow the note and explicitly mention the conflict in your reply (e.g. "Memory note #N says proxydroid uses `master` as default branch — using `master` for base, not `main`."). Never silently ignore either the note or the Skill rule.
...
Why: the extension strips unregistered tags without error. The user has already lost hours to this exact bug.
...
1. `get_workflow_run_status(owner, repo, run_id)` — first, to see which jobs and steps failed. This is non-blocking and cheap.
2. `get_workflow_run_logs(owner, repo, run_id)` with `grep_pattern` (e.g. `'e: file|Unresolved|error:|Caused by'`) and `tail_lines=0` — this returns only matching lines, not megabytes of log. Prefer this over `get_full_workflow_logs`, which is often truncated by the client.
3. If the failing step is known, `get_run_logs_by_step(step_name, direction='tail')` for the end of that step.
4. If the error is in the middle, `read_run_logs_offset(offset, limit)` for a specific window.

Do NOT call `get_full_workflow_logs` on a large run — the answer will be truncated and you will miss the real cause.

For Android/iOS builds specifically, prefer the dedicated helpers: `get_android_build_error`, `get_ios_build_error`, `auto_fix_build`.

## Rule 7 — Do not touch project architecture without reading it first

The user's `mcp-server` has a specific plugin architecture:
...
NEVER create a parallel file (like the deleted `files.py`) that nobody imports. Code that lives in an unwired file is dead code — the tool will not exist even though the file compiles.
...
- **Test-first for tool changes**: any new or changed `@mcp_tool` function gets a small pytest that calls it with a mocked GitHub client and asserts the return shape. No exception.
- **Contract tests for tools/list**: a single test that calls `list_tools()` and asserts the count and that all names are unique. This catches accidental duplicates and drift between code and registration.
- **Idempotency for write tools**: file-update tools must accept (or auto-fetch) SHA; branch-creation must be idempotent (return existing SHA on conflict, no error).
- **Small, single-purpose modules**: one topic per file, per the existing convention. No god-files.
- **Typed return envelopes**: tools should return either a serialisable dict or a string — never a Python object that the MCP transport cannot serialise.
- **Explicit `description` budgets**: keep `description` short (1-2 sentences) since the plugin UI renders them and the user reads them. Do NOT shorten parameter descriptions; only the top-level tool description.
